...
(function (root, factory) {
  ...
  function schedule(G1, ctx, anchor, offset, correctionMs, beeps, spacingS) {
    return G1.schedule(anchor, offset, correctionMs, { family: ctx.timing, beeps: beeps, spacingS: spacingS, resetModel: anchor === 'reset' ? ctx.resetModel : undefined, methodology: ctx.meth });
  }
  ...
  function protocolLines(G1, D, ctx, anchor, s, aimed, correction, beeps, spacing, fmt) {
    ...
    var holdLo = G1.framesToSeconds(timing.hold_lo_frame), holdHi = G1.framesToSeconds(timing.hold_hi_frame), menuS = G1.framesToSeconds(timing.menu_frame);
    ...
    if (anchor === 'menu') {
      ...
      out.push('The NEW GAME menu opens at ' + fmt.s(menuS, 2) + ' (frame ' + timing.menu_frame + '). The INSTANT you see it, tap the Run button. Release START whenever you like; release timing does not matter.');
      out.push('You will hear ' + beeps + ' short beep' + (beeps === 1 ? '' : 's') + ' ' + spacing.toFixed(1) + ' s apart (white flashes with the numerals), then one long high beep with a green "A!". Press A ON the long high beep. That is the only frame-exact action.' +
        (aimed ? ' Target: A at ' + fmt.s(G1.targetSeconds(aimed.offset)) + ' after the menu (frame ' + G1.pressFrameFromMenu(aimed.offset) + ' after it). The beep is ' + fmt.ms(correction) + ' early to cover your reaction to the menu plus the audio delay (the correction; the calibration tunes it).' : ' Pick a target to see the numbers.'));
    } else {
      ...
      if (s && s.holdLo !== null && s.holdHi !== null && s.menu !== null) {
        ...
        out.push('A double blip (amber "MENU") at ' + fmt.s(s.menu, 2) + ' marks when the NEW GAME menu should appear (frame ' + timing.menu_frame + ' of the boot). If the menu appears far from the blip, START was held outside the window: the attempt is no good, reset and try again. Release START whenever.');
  ...
  var pure = { context: context, schedule: schedule, routeValid: routeValid, invertTid: invertTid, protocolLines: protocolLines, submitGot: submitGot };
  ...
  if (A && typeof document !== 'undefined') {
    ...
    function targetsHtml(game, ctx) {
      ...
      if (mode === 'set') {
        ...
        }).join('') + '</table>' : '<p class="warn">No offset of this table produces a member of the sets in force.</p>';
      } else {
    ...
    function statusHtml(ctx) {
      ...
      return A.statusBlock([
        ...
        A.details('Protocol as the data states it', '<p class="small">' + esc(m.protocol) + '</p>') + A.details('Validation and derivation', '<p class="small">' + esc(m.validation) + '</p><p class="small">' + esc(m.derivation) + '</p>');
    }
    function render(el, game) {
      ...
      h += A.card('<h3>6. Cue and storyboard</h3>' + (ctx.schedError ? '<p class="bad">' + esc(ctx.schedError) + '</p>' : ctx.sched ? '<p class="small muted">A cue at ' + A.fmtS(ctx.sched.tA) + ' after the anchor; ' + ctx.sched.cues.length + ' tones.</p>' + A.widgets.storyWidgetHtml('g1-story', ANCHOR_BUTTON[ctx.anchor], null) : '<p class="muted">Pick a target to build the cue.</p>') +
